...
import adminNotificacionesService from "../services/adminNotificacionesService";
...
function AdminLayout() {
  ...
  const cargarNotificaciones = async () => {
    try {
      const response = await adminNotificacionesService.getNotificaciones({ limit: 10 });
  ...
  const handleMarcarLeida = async (id) => {
    try {
      await adminNotificacionesService.marcarComoLeida(id);
  ...
  const handleMarcarTodasLeidas = async () => {
    try {
      await adminNotificacionesService.marcarTodasComoLeidas();
      await cargarNotificaciones();
    } catch (error) {
