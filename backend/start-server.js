...
try {
  ...
  // Middlewares
  app.use(cors());
  app.use(express.json({ limit: '10mb' })); // Aumentar límite para subir imágenes en base64
  ...
  try {
    ...
    console.log('  ✅ notificaciones.routes');
  } catch (err) {
    console.error('  ❌ Error en notificaciones.routes:', err.message);
  }
